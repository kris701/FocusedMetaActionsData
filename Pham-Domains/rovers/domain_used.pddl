...
;navigate_mcr_sample_soil remove ?y
(:action dynamicmacro_mod_2 :parameters (?x - rover ?z - waypoint ?sx3 - store)
...
;calibrate_mcr_take_image_mcr_communicate_image_data remove ?yx6x7 ?w ?lx5x6 ?t
(:action dynamicmacro_merged_1 :parameters (?r - rover ?i - camera ?ox4 - objective ?mx5 - mode)
:precondition (and (equipped_for_imaging ?r)
(on_board ?i ?r)
(supports ?i ?mx5)
(available ?r)
)
:effect (and (have_image ?r ?ox4 ?mx5)
(communicated_image_data ?ox4 ?mx5)
(not (calibrated ?i ?r))
)
)
