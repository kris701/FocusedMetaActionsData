...
;turn_to_mcr_take_image remove ?s
(:action dynamicmacro_eff_2 :parameters (?d_new ?ix3 ?mx4)
:precondition (and 
(instrument ?ix3) (direction ?d_new) (mode ?mx4)
(calibrated ?ix3)
(supports ?ix3 ?mx4)
(power_on ?ix3)
)
...
;turn_to_mcr_take_image remove ?ix3
(:action dynamicmacro_pre_1 :parameters (?s ?d_new ?d_prev ?mx4)
:precondition (and 
(satellite ?s) (direction ?d_new) (direction ?d_prev) (mode ?mx4)
(pointing ?s ?d_prev)
)
:effect (and (have_image ?d_new ?mx4)
(pointing ?s ?d_new)
...
;switch_on_mcr_calibrate_mcr_turn_to_mcr_take_image remove ?dx2
(:action dynamicmacro_mod_5 :parameters (?i ?s ?d_newx3x3 ?mx4x4x4)
:precondition (and 
(satellite ?s) (direction ?d_newx3x3) (instrument ?i) (mode ?mx4x4x4) 
(on_board ?i ?s)
(power_avail ?s)
(supports ?i ?mx4x4x4)
)
