...
;move_mcr_move remove ?nextpos
(:action dynamicmacro_eff_9 :parameters (?curpos - object ?nextposx2 - object)
:precondition (and (place ?curpos)
(at-robot ?curpos)
(place ?nextposx2)
(open ?nextposx2)
)
:effect (and (at-robot ?nextposx2)
(not (at-robot ?curpos))
)
)
...
;move_mcr_pickup remove ?curpos
(:action dynamicmacro_mod_3 :parameters (?nextpos - object ?keyx2 - object)
:precondition (and (open ?nextpos)
(key ?keyx2)
(at ?keyx2 ?nextpos)
(arm-empty)
)
:effect (and (holding ?keyx2)
