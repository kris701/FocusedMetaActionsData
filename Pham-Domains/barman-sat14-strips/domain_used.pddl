...
;dynamicmacro_shake_mcr_empty-shaker_both
(:action dynamicmacro_merged_2 :parameters (?b - cocktail ?d1 - ingredient ?d2 - ingredient ?s - shaker ?lx6 - level ?l1x7 - level)
:precondition (and 
(contains ?s ?d1)
(contains ?s ?d2)
(cocktail-part1 ?b ?d1)
...
:effect (and (shaker-level ?s ?l1x7)
(empty ?s)
(not (unshaked ?s))
(not (contains ?s ?d1))
(not (contains ?s ?d2))
