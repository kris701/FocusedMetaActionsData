(define
	...
	(:action dynamicmacro_merged_1
		:parameters (?s - shot ?i - ingredient ?d - dispenser ?dx5 - shaker ?lx6 - level ?l1x7 - level)
		:precondition 
			(and
				(dispenses ?d ?i)
				(empty ?s)
	...
	(:action dynamicmacro_merged_2
		:parameters (?b - cocktail ?d1 - ingredient ?d2 - ingredient ?s - shaker ?lx6 - level ?l1x7 - level)
		:precondition 
			(and
				(contains ?s ?d1)
				(contains ?s ?d2)
